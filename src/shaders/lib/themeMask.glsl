...
// background). While the Portal theme switch runs, each material mixes its OLD theme values
// toward the NEW ones by this per-fragment "newness" (0 = still the old theme,
...
float themeMaskNewness() {
    ...
    // Portal: the front is a circle growing from the screen centre (corner-normalized radius).
    vec2 p = screenUv - 0.5;
    p.x *= aspect;
    float maxR = 0.5 * sqrt(aspect * aspect + 1.0);
    float rN = length(p) / maxR;
    float front = uThemeMaskProgress * (1.0 + uThemeMaskBand);
    float t = clamp((front - rN) / max(uThemeMaskBand, 0.0001), 0.0, 1.0);
